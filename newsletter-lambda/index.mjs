...
// Slack webhook for notifications (set as env variable)
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (path.endsWith("/unsubscribe") && method === "GET") {
...
async function handleSubscribe(event) {
  ...
  if (existing.Item?.confirmed) {
    return {
      ...
      headers,
      body: JSON.stringify({
        message: "Ten email jest już zapisany na newsletter",
      }),
    };
  }
...
async function handleConfirm(event) {
  ...
  await notifySlack(
    `🎉 Nowy subskrybent newsletter 1copywriting.pl!\n📧 ${normalizedEmail}${name}`,
  );
...
async function handleUnsubscribe(event) {
  ...
  // Notify admin via Slack
  await notifySlack(
    `👋 Ktoś wypisał się z newslettera 1copywriting.pl\n📧 ${normalizedEmail}`,
  );
